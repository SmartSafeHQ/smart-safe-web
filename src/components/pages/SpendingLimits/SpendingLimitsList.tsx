...
import { SpendingLimitsTable } from '@components/pages/SpendingLimits/SpendingLimitsTable'
...
export function SpendingLimitsList() {
  ...
  return (
    <ScrollArea>
      {error ? (
        <ErrorState
          title="Could not load your spending limit. Please try again."
          description={(error as Error)?.message ?? 'Internal server error'}
          className="mt-10"
        />
      ) : isLoading ? (
        <LoadingState title="Loading spending limits" className="mt-10" />
        ...
        <div className="w-full flex flex-col items-center gap-1 pt-8 text-center">
          <Text asChild className="text-lg font-medium">
            <strong>No spending limit added!</strong>
          </Text>
        </div>
      ) : (
        spendingLimits && (
