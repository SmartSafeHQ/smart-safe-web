...
function DialogModalIconClose({ Icon = X }: DialogModalIconCloseProps) {
  return (
    <RadixDialog.Close
      asChild
      className="flex items-center justify-center p-2 absolute top-6 right-8 text-gray-400 text-2xl bg-transparent rounded-md hover:text-gray-50 hover:bg-gray-500/10 transition-colors"
    >
  ...
  return (
    <RadixDialog.Portal className="relative" {...props}>
      ...
      <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50 p-0 md:p-8">
        <RadixDialog.Content
          className={clsx(
            'w-full h-full max-w-full overflow-auto bg-gray-800 shadow-lg text-gray-50 animate-dialog-open md:h-[50vh]  md:rounded-md',
            className
          )}
        >
